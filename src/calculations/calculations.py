...
def do_calculations(correct_answer_dict: Dict[str, str], questions_class_list: List[q.Question],
    ...
    [question.set_correct_answer(correct_answer_dict[question.question_number])
     for question in questions_class_list if question.question_number in correct_answer_dict.keys()]

    # populate is_correct through responses
    [response.set_is_correct(question.correct_answer, question.points) for question in questions_class_list
     for response in question.list_of_responses if question.correct_answer is not None]
